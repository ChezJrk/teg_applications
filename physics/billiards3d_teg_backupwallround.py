...
def solve_teg(prob: bc.BilliardsProblem, a:ITeg) -> Tuple[Optional[bc.Path], List[ITeg]]:
    ...
    J = np.array([17.442, 22.166]) * scalefactor
    K = np.array([-8.411, 32.025]) * scalefactor
    mint = args.mint
    maxt = args.maxt

    twall = Var('twall', mint + (maxt-mint)*0.1)
    ton = Var('ton', mint + (maxt-mint)*0.2)
    tup = Var('tup', mint + (maxt-mint)*0.5)
    toff = Var('toff', mint + (maxt-mint)*0.7)
    tst = Var('tst', mint + (maxt-mint)*0.8)
    tuv = Var('tuv', mint + (maxt-mint)*0.9)
    xwall = Var('xwall', (K[0] + J[0])/2)
    ...
    key_points = [
        (mint, tee[0], tee[1]),
        (twall, xwall, remap(xwall, K[0], J[0], K[1], J[1])),
        (ton, xon, zon),
        # (tup, xup, zup),
        (toff, xoff, zoff),
        # (tst, S[0], zst),
    ...
    key_hints = [
        (mint, tee[0], tee[1]),
        (twall.value, xwall.value, remap(xwall.value, K[0], J[0], K[1], J[1])),
