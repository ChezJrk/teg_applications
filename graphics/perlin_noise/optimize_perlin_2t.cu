...
{
    if (argc != 6) {
        std::cout << "Usage: ./optimize_perlin_2t <image> <grid-nx> <grid-ny> <seed> <lrate>" << std::endl;
        exit(1);
    }
    ...
    */
    for (int iter = 0; iter < 300; iter ++){
        ...
        */
        ...
        perlin_double_threshold_kernel<<<(job_count / 256) + 1, 256>>>(
                            qids,
                            pids,
                            job_count,
                            grid,
        ...
        *d_mcolor = 0.f;
        ...
        perlin_double_threshold_deriv<<<(job_count / 256) + 1, 256>>>(
                            qids,
                            pids,
                            job_count,
                            target_image,
        ...
        update_grid_positions<<<dimGrid, dimBlock>>>(grid, d_grid, alpha_vertex);
        ...
        mcolor->r = mcolor->r - d_mcolor->r * ALPHA_COLOR;
        mcolor->g = mcolor->g - d_mcolor->g * ALPHA_COLOR;
        mcolor->b = mcolor->b - d_mcolor->b * ALPHA_COLOR;
        */

        //*threshold = *threshold - (*d_threshold) * ALPHA_THRESHOLD;
        //*threshold2 = *threshold2 - (*d_threshold2) * ALPHA_THRESHOLD;
