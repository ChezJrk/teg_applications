...
#define ALPHA_COLOR 0.5
#define ALPHA_VERTEX 3000

int main(int argc, char** argv)
{
    if (argc != 5) {
        std::cout << "Usage: ./triangulate_quadratic <image_file> <tri-grid nx> <tri-grid ny> <use-deltas:y/n>" << std::endl;
        exit(1);
    }
    ...
    for (int iter = 0; iter < 150; iter ++){
        ...
        if (use_deltas) {
            ...
            quadratic_deriv_kernel_nodelta<<<(num_jobs / 256) + 1, 256>>>(
                tids,
                pids,
                num_jobs,
                tri_image,
        ...
        quadratic_loss_kernel<<<(num_jobs / 256) + 1, 256>>>(
            tids,
            pids,
            num_jobs,
            tri_image,
        ...
        compute_triangle_regularization(mesh, d_mesh, 5);
